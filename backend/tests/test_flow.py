...
from app.services.phase_manager import (
    Phase, MigrantType, PhaseManager, get_phase_manager,
    PHASE_CONFIG, PRICES
)
from app.services.off_topic_detector import (
    OffTopicDetector, MessageType, get_detector,
...
class TestPhaseManager:
    ...
    def test_payment_tracking(self, phase_manager):
        ...
        # Sin pago
        assert phase_manager.is_phase_paid(user_id, Phase.DIAGNOSTICO) == False
        
        # Marcar pago
        phase_manager.mark_payment(user_id, Phase.DIAGNOSTICO)
        assert phase_manager.is_phase_paid(user_id, Phase.DIAGNOSTICO) == True
...
class TestOffTopicDetector:
    ...
    def test_helper_functions(self):
        ...
        assert is_confirmation("sí") == True
        assert is_confirmation("no") == False
        assert is_rejection("no") == True
        assert is_rejection("sí") == False
...
class TestDeliverables:
    ...
    def test_registration_deliverable(self, deliverable_generator, test_user_data):
        ...
        assert deliverable.is_pdf == False
