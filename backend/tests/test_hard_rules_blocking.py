...
from app.services.memory_profiler import CorrectionTracker, can_make_decision, detect_correction
from app.services.never_silent import EMPATHIC_RECOVERY_MESSAGES, STILL_HERE_MESSAGES
        ...
        if can_show:
            ...
            if interactions >= 5:
                ...
                if ratio > 0.25:  # Más de 1/5 = 20% + margen
                    violations.append(f"Interaction {interactions}: {forms_shown} forms ({ratio*100:.0f}%)")
        else:
...
def test_rule_3_visa_without_context():
    ...
    complete_profile = {
        "profile": {
            ...
            "family": {"family_status": "single"},
            "history": {"has_visas": False, "criminal_record": False},
        },
        "family_status": "single",
        "family_members": [],
        "preferences": {"migration_reason": "Mejores oportunidades"},
    }
    ...
    return TestResult(
        ...
        passed=passed,
        message=(
            f"{len(incomplete_profiles)} incomplete profiles blocked"
            if passed
...
def test_rule_4_understanding_required():
    ...
    confirmed_profile["confirmations"] = [{"summary_type": "profile", "confirmed": True}]
    ...
    return TestResult(
        ...
        passed=passed,
        message="All critical decisions require confirmation" if passed else f"{len(violations)} violations",
        details=violations,
    )
...
def test_rule_5_corrections_not_ignored():
    ...
    return TestResult(
        ...
        passed=passed,
        message="All corrections detected and block advance" if passed else f"{len(failures)} failures",
        details=failures,
    )
...
def test_rule_6_markdown_valid():
    ...
    return TestResult(
        ...
        passed=passed,
        message=(
            f"All {len(messages_to_check)} messages have valid markdown"
            if passed
            else f"{len(failures)} invalid"
        ),
        details=failures,
    )
...
def test_fuzz_100_random_inputs():
    ...
    return TestResult(
        ...
        passed=passed,
        message="100/100 inputs handled" if passed else f"{len(failures)} failures",
        details=failures[:5],
    )
...
def test_20_complete_flows():
    ...
    return TestResult(
        ...
        passed=passed,
        message="20/20 flows completed" if passed else f"{len(failures)} flows failed",
        details=failures[:5],
    )
