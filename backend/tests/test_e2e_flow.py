...
from app.services.ai_brain import fallback_response, get_fallback_message
from app.services.deliverables import DeliverableGenerator
from app.services.off_topic_detector import MessageType, get_detector
from app.services.phase_manager import PHASE_CONFIG, Phase, PhaseManager
from app.services.ux_helpers import get_payment_invitation, get_progress_header
...
def test_complete_flow():
    ...
    test_data = {
        ...
        Phase.EJECUCION: {"documents_collected": True, "forms_completed": True, "evidence_prepared": True},
    }
...
def test_fallbacks():
    ...
    user_data = {
        "profile": {"personal": {"name": "Test User"}, "work": {"experience": ">10 años"}},
...
def test_blocking_without_data():
    ...
    can_advance, msg = pm.can_advance(user_id)
    assert can_advance is False, "No debería poder avanzar sin datos"
    ...
    can_advance, msg = pm.can_advance(user_id)
    assert can_advance is False, "No debería poder avanzar con datos incompletos"
...
def test_payment_gating():
    ...
    can_advance, msg = pm.can_advance(user_id)
    assert can_advance is False, "No debería poder avanzar sin pago"
    ...
    can_advance, msg = pm.can_advance(user_id)
    assert can_advance is True, f"Debería poder avanzar después del pago: {msg}"
