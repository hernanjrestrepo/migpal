...
class TestIntentDetection:
    ...
    def test_detect_question(self):
        ...
        assert intent in [UserIntent.ASKING_QUESTION, UserIntent.WANTS_INFO], \
...
class TestConversationalResponses:
    ...
    async def test_not_ready_response_is_empathic(self):
        ...
        response = await ai.process_free_text(
            "no estoy listo para escoger una zona",
            user,
            "location_region",
            "es"
        )
    ...
    async def test_confused_response_explains(self):
        ...
        user = {
            "profile": {"personal": {"name": "María"}},
            "language": "es"
        }
        
        response = await ai.process_free_text(
            "no conozco Estados Unidos, no sé qué zona elegir",
            user,
            "location_region",
            "es"
        )
        ...
        assert any(word in response.message.lower() for word in ["región", "sur", "norte", "oeste", "florida", "california"])
...
class TestNoErrorMessages:
    ...
    def test_telegram_bot_no_error_message(self):
        ...
        for match in matches:
            ...
            assert "conversational_ai" in content or "Conversational AI" in content, \
