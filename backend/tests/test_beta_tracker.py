...
from app.services.beta_tracker import (
    BetaTracker, EventType, get_beta_tracker,
    log_event, is_beta_mode, generate_beta_report,
    request_feedback, process_feedback
)
from app.services.beta_integration import (
    add_beta_user, is_beta_user, get_beta_welcome_message,
    on_phase_start, on_phase_complete, on_message_received,
    on_frustration_detected, on_payment_prompted, on_payment_completed,
    format_beta_message, get_beta_status
)
...
def test_beta_tracker_initialization():
    ...
    tracker = BetaTracker()
    assert tracker.enabled == True, "Beta mode debería estar habilitado"
...
def test_feedback_system():
    ...
    assert success == True, "Feedback no procesado"
...
def test_beta_integration():
    ...
    assert result == True, "Usuario no agregado"
    
    # Verificar que es usuario beta
    assert is_beta_user(test_user) == True, "Usuario no reconocido como beta"
