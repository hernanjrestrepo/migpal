...
class MigPALConsultingEngine:
    ...
    def get_state(self, user_id: int) -> ConsultingState:
        ...
        if user_id not in self._states:
            self._states[user_id] = ConsultingState(
                user_id=user_id,
    ...
    def get_next_profiling_question(self, user_id: int) -> Optional[Dict]:
    ...
    def _generate_diagnostic_summary(self, score: int, is_viable: bool, strengths: List[str], weaknesses: List[str]) -> str:
...
"""
    ...
    async def research_city(self, user_id: int, city: str, state_code: str) -> Dict:
        ...
        profile = ClientProfile(
            ...
            children=[{"name": "hijo", "age": 10}] if consulting_state.profile.get("preferences", {}).get("family_status") == "con_familia" else [],
    ...
    async def run_due_diligence(self, user_id: int, business_id: str) -> Dict:
        ...
        report = {
            ...
            "financial_analysis": {
                ...
                "revenue_multiple": business.get("asking_price", 0) / max(business.get("annual_revenue", 1), 1),
            },
            "risk_assessment": {
                "market_risk": "Medio",
            ...
            "verdict": "RECOMENDADO" if business.get("annual_profit", 0) / max(business.get("asking_price", 1), 1) > 0.2 else "REVISAR CON CUIDADO",
        }
...
"""
    ...
    async def generate_migration_plan(self, user_id: int) -> Dict:
        ...
        plan = {
            ...
            "profile_summary": {
                ...
                "city": state.selected_city or (state.researched_cities[0] if state.researched_cities else ""),
            },
            ...
            # Negocio (si aplica)
            "business": {
                "recommendations": state.businesses_found[:3] if state.preferences.get("work_preference") in ["negocio", "ambos"] else [],
            },
    ...
    def _generate_timeline(self, state: ConsultingState) -> List[Dict]:
        ...
        return [
            {"month": 1, "phase": "Preparación", "tasks": ["Reunir documentos", "Preparar evidencia", "Traducciones"]},
...
_consulting_engine: Optional[MigPALConsultingEngine] = None
...
def format_school_list(schools: List[Dict], limit: int = 5) -> str:
    ...
    for i, school in enumerate(schools[:limit], 1):
        ...
        stars = "⭐" * int(rating / 2)
        type_emoji = {"public": "🏫", "private": "🎒", "charter": "📚"}.get(school.get("school_type", ""), "🏫")
