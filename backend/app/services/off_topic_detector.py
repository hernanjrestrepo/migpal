...
MIGRATION_KEYWORDS = [
    # Visas
    "visa", "visado", "h1b", "h-1b", "l1", "l-1", "o1", "o-1", "e2", "e-2",
    "eb1", "eb-1", "eb2", "eb-2", "eb5", "eb-5", "green card", "residencia",
    "ciudadanía", "ciudadania", "naturalización", "naturalizacion",
    
    # Proceso migratorio
    "migrar", "migración", "migracion", "emigrar", "inmigrar", "mudarse",
    "uscis", "embajada", "consulado", "entrevista consular", "petición",
    "patrocinador", "sponsor", "sponsorship",
    
    # Ubicación USA
    "usa", "estados unidos", "eeuu", "america", "americano",
    "florida", "miami", "texas", "houston", "california", "los angeles",
    "new york", "nueva york", "chicago", "boston", "seattle", "austin",
    "orlando", "atlanta", "denver", "phoenix", "san francisco",
    
    # Trabajo
    "trabajo", "empleo", "empresa", "salario", "sueldo", "profesión",
    "experiencia laboral", "carrera", "industria", "sector",
    
    # Negocio
    "negocio", "empresa", "emprender", "inversión", "inversion", "capital",
    "franquicia", "startup",
    
    # Vivienda
    "vivienda", "casa", "apartamento", "departamento", "renta", "alquiler",
    "comprar casa", "barrio", "vecindario", "zona",
    
    # Educación
    "escuela", "colegio", "universidad", "educación", "educacion",
    "estudios", "hijos", "niños", "familia",
    
    # Documentos
    "documento", "pasaporte", "certificado", "título", "diploma",
    "antecedentes", "récord", "historial",
    
    # Costos
    "costo", "precio", "cuánto", "cuanto", "pagar", "pago", "tarifa",
    "presupuesto", "ahorro", "dinero",
    
    # Proceso MigPAL
    "migpal", "diagnóstico", "diagnostico", "perfilamiento", "plan",
    "fase", "etapa", "paso", "siguiente", "continuar", "avanzar",
]

# Confirmaciones
CONFIRMATION_WORDS = [
    "sí", "si", "ok", "okay", "dale", "listo", "bueno", "vale", "claro",
    "perfecto", "de acuerdo", "entendido", "correcto", "exacto", "así es",
    "adelante", "vamos", "hagámoslo", "hagamoslo", "procede", "continúa",
    "continua", "siguiente", "next", "yes", "sure", "go", "proceed"
]

# Rechazos
REJECTION_WORDS = [
    "no", "nop", "nope", "después", "despues", "luego", "más tarde",
    "mas tarde", "ahora no", "otro día", "otro dia", "no gracias",
    "no quiero", "no puedo", "cancelar", "parar", "detener", "stop"
]

# Saludos
GREETING_WORDS = [
    "hola", "hello", "hi", "hey", "buenos días", "buenos dias",
    "buenas tardes", "buenas noches", "qué tal", "que tal",
    "cómo estás", "como estas", "saludos", "buen día", "buen dia"
]

# Frustración
FRUSTRATION_WORDS = [
    "no entiendo", "no entiendes", "ya te dije", "otra vez", "de nuevo",
    "no me escuchas", "frustrante", "molesto", "cansado", "harto",
    "nojoda", "carajo", "mierda", "diablos", "rayos", "demonios",
    "no friegues", "deja de", "basta", "para ya"
]

# Ayuda
HELP_WORDS = [
    "ayuda", "help", "auxilio", "no sé", "no se", "estoy perdido",
    "confundido", "no entiendo", "explícame", "explicame", "qué hago",
    "que hago", "cómo funciona", "como funciona"
]

# Pagos
PAYMENT_WORDS = [
    "pagar", "pago", "tarjeta", "paypal", "zelle", "transferencia",
    "stripe", "precio", "costo", "factura", "recibo", "comprobante"
]
...
class OffTopicDetector:
    ...
    def _compile_patterns(self):
        ...
        self.migration_pattern = re.compile(
            r'\b(' + '|'.join(re.escape(w) for w in MIGRATION_KEYWORDS) + r')\b',
            re.IGNORECASE
        )
        ...
        self.frustration_pattern = re.compile(
            r'\b(' + '|'.join(re.escape(w) for w in FRUSTRATION_WORDS) + r')\b',
            re.IGNORECASE
        )
        self.help_pattern = re.compile(
            r'\b(' + '|'.join(re.escape(w) for w in HELP_WORDS) + r')\b',
            re.IGNORECASE
        )
        self.payment_pattern = re.compile(
            r'\b(' + '|'.join(re.escape(w) for w in PAYMENT_WORDS) + r')\b',
            re.IGNORECASE
        )
...
def get_off_topic_response(
    ...
    message_type: MessageType = MessageType.OFF_TOPIC
) -> str:
    """
...
_detector: Optional[OffTopicDetector] = None
