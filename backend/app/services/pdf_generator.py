...
class MigPALReport(FPDF):
    ...
    def footer(self):
        ...
        self.set_text_color(128, 128, 128)
        self.cell(0, 10, f'Page {self.page_no()}/{{nb}} | Generated: {datetime.now().strftime("%Y-%m-%d %H:%M")} | MigPAL - t.me/MigPAL_Bot', 0, 0, 'C')
...
def generate_case_report_pdf(user_data: Dict[str, Any], user_id: int) -> Optional[str]:
    """
    ...
    try:
        ...
        if documents:
            ...
            for i, doc in enumerate(documents, 1):
                pdf.body_text(f"{i}. {doc.get('file_name', doc.get('type', 'Document'))} - Uploaded: {doc.get('uploaded_at', 'N/A')}")
...
def generate_checklist_pdf(user_data: Dict[str, Any], checklist: list, user_id: int) -> Optional[str]:
