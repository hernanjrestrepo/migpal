...
class JobListing:
    ...
    def format_for_user(self) -> str:
        salary_str = (
            f"${self.salary_min:,} - ${self.salary_max:,}/año"
            if self.salary_min
...
class CommunityInfo:
    ...
    def format_for_user(self) -> str:
        safety = (
            ...
            else "🟡 Moderado" if self.crime_index < 60 else "🔴 Precaución"
        )
...
class MigPALResearchEngine:
    ...
    async def search_properties(
        ...
        try:
            ...
            response = await client.get(
                f"https://{ZILLOW_API_HOST}/propertyExtendedSearch", params=params, headers=headers
            )
    ...
    def _get_sample_properties(
        self, city: str, state: str, listing_type: str, limit: int
    ) -> list[PropertyListing]:
        ...
        prices = city_prices.get(
            city_key, {"rent_1br": 1500, "rent_2br": 2000, "rent_3br": 2500, "sale": 350000}
        )
    ...
    async def search_jobs(
        ...
        try:
            ...
            response = await client.get(
                f"https://{INDEED_API_HOST}/jobs/search", params=params, headers=headers
            )
    ...
    def _get_sample_jobs(self, query: str, location: str, limit: int) -> list[JobListing]:
        ...
        company_list = companies.get(
            "tech" if "tech" in query_lower or "software" in query_lower else "default", companies["default"]
        )
    ...
    def _get_sample_schools(
        self, city: str, state: str, grade_level: str, school_type: str, limit: int
    ) -> list[SchoolInfo]:
        ...
        city_key = city.lower().replace(" ", "_")
        school_list = schools_by_city.get(
            city_key,
            [
    ...
    def _get_sample_businesses(
        self, location: str, business_type: str, max_price: int, limit: int
    ) -> list[BusinessListing]:
        ...
        for i, biz in enumerate(business_types[:limit]):
            ...
            businesses.append(
                BusinessListing(
                    ...
                    description="Negocio establecido con clientela fiel. Excelente oportunidad para emprendedor.",
    ...
    async def get_community_info(self, city: str, state: str, neighborhood: str = None) -> CommunityInfo:
        """
    ...
    def _get_sample_community(self, city: str, state: str, neighborhood: str) -> CommunityInfo:
        ...
        return CommunityInfo(
            ...
            description="Comunidad diversa con fuerte presencia latina. Excelente para familias hispanas.",
        )

    # ============== INVESTIGACIÓN COMPLETA ==============

    async def full_research(self, profile: ClientProfile, city: str, state: str) -> dict[str, Any]:
        """
        ...
        if profile.children:
            tasks.append(
                self.search_schools(
                    city=city, state=state, school_type=profile.school_type_preference, limit=5
                )
            )
        ...
        if profile.work_preference in ["negocio", "ambos"]:
            tasks.append(
                self.search_businesses(
                    location=f"{city}, {state}", max_price=profile.business_budget or 200000, limit=5
                )
            )
        ...
        # Negocios
        if (
            profile.work_preference in ["negocio", "ambos"]
            and idx < len(task_results)
            and not isinstance(task_results[idx], Exception)
        ):
        ...
        if idx < len(task_results) and not isinstance(task_results[idx], Exception):
            results["community"] = (
                task_results[idx].__dict__ if hasattr(task_results[idx], "__dict__") else {}
            )
...
_research_engine: MigPALResearchEngine | None = None
...
def get_next_deep_question(answered_questions: list[str]) -> str | None:
