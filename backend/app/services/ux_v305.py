...
class CorrectionNLU:
    ...
    CORRECTION_KEYWORDS = [
        "correcto", "corregir", "cambiar", "actualizar", "perdón", "perdon",
        "disculpa", "error", "equivoqué", "equivoque", "mal", "incorrecto",
        "correct", "change", "update", "sorry", "wrong", "mistake"
    ]
    
    @classmethod
    def detect_correction(cls, text: str) -> Optional[CorrectionIntent]:
        """
...
class ConversationalOnboarding:
    ...
    @classmethod
    def get_question_with_other(cls, question: str, options: List[Tuple[str, str]], lang: str = "es") -> Tuple[str, List[Tuple[str, str]]]:
        """
        ...
        """
        other_option = ("✏️ Otro (escribir)", "other_write") if lang == "es" else ("✏️ Other (write)", "other_write")
...
class MultiSelect:
    ...
    @classmethod
    def create_multi_select_keyboard(cls, options: List[Tuple[str, str]], selected: List[str] = None) -> List[List[Tuple[str, str]]]:
        """
...
def log_user_interaction(user_id: int, interaction_type: str, details: Dict[str, Any]):
    ...
    logger.info(
        f"👤 USER_INTERACTION | user={user_id} | type={interaction_type} | "
        f"details={details}"
    )
...
def log_markdown_error(user_id: int, original_text: str, error: str):
    ...
    logger.warning(
        f"⚠️ MARKDOWN_ERROR | user={user_id} | error={error} | "
        f"text_preview={original_text[:50]}..."
    )
