...
class StateDecision:
    ...
    next_phase: Optional[ProgressPhase] = None
...
class StateValidationLayer:
    ...
    def _get_missing_critical_info(self) -> List[str]:
        ...
        for info_key in self.CRITICAL_FOR_SUMMARY:
            if info_key == "family_situation":
                if not self.understanding.get("migrating_alone") and not self.understanding.get("family_members"):
                    missing.append("situación familiar")
            elif info_key == "work_situation":
                if not self.understanding.get("current_profession") and not self.understanding.get("current_income"):
    ...
    def get_summary_for_confirmation(self) -> Optional[str]:
        """
        ...
        if self.understanding.get("migrating_alone"):
            ...
            if isinstance(family, list):
                ...
                parts.append(f"Viajarías con tu familia")
    ...
    def _has_minimum_for_summary(self) -> bool:
        ...
        has_family = bool(self.understanding.get("migrating_alone") or self.understanding.get("family_members"))
