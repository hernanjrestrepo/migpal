...
class PaymentManager:
    ...
    def create_payment(
        self, user_id: int, payment_type: PaymentType, method: PaymentMethod = None
    ) -> PaymentRecord:
        """Crea un nuevo registro de pago"""
        amount = PRICES.get(payment_type, 0)

        record = PaymentRecord(user_id=user_id, payment_type=payment_type, amount=amount, method=method)
    ...
    def get_payment_link(
        self, user_id: int, payment_type: PaymentType, method: PaymentMethod
    ) -> tuple[str, str]:
        """
...
Una vez enviado, confirma aquí."""
    ...
    def confirm_payment(
        self, payment_id: str, transaction_id: str = None, metadata: dict = None
    ) -> tuple[bool, str]:
    ...
    def create_refund(self, user_id: int, reason: str, amount: float = None) -> tuple[bool, str, str]:
        """
    ...
    def get_user_total_paid(self, user_id: int) -> float:
        ...
        return sum(p.amount for p in payments if p.status == PaymentStatus.COMPLETED and p.amount > 0)
...
_payment_manager: PaymentManager | None = None
