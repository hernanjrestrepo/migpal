...
def _load_v4_modules():
    ...
    if _standard is None:
        try:
            from . import migpal_usa_standard
            from . import migpal_usa_integration
...
@safe_v4_call
def pre_process_message(user_id: int, text: str, state: str, user_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    """
...
@safe_v4_call
def get_progress_indicator(user_id: int) -> Optional[str]:
    """
...
@safe_v4_call
def evaluate_states(user_id: int, custom_weights: Optional[Dict[str, float]] = None) -> Optional[str]:
    """
...
@safe_v4_call
def evaluate_cities(state: str, user_id: int, custom_weights: Optional[Dict[str, float]] = None) -> Optional[str]:
    """
...
@safe_v4_call
def evaluate_businesses(user_id: int, budget: float = 150000, 
                       custom_weights: Optional[Dict[str, float]] = None) -> Optional[str]:
    """
...
@safe_v4_call
def get_document_checklist(visa_type: str) -> Optional[str]:
...
@safe_v4_call
def get_installation_checklist() -> Optional[str]:
...
@safe_v4_call
def get_interview_prep(visa_type: str) -> Optional[Dict[str, Any]]:
...
@safe_v4_call
def get_micro_check(lang: str = "es") -> Optional[str]:
...
class V4GatingEnforcer:
    ...
    def check(self) -> Tuple[bool, str]:
        ...
        try:
            self.can_recommend, self.blocking_message = check_visa_gating(
