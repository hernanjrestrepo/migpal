                ...
                if not self._response_sent.get(user_id, False):
                    ...
                    if self._can_send_watchdog_message(user_id):
                        ...
                        try:
                            ...
                            logger.info(f"⏱️ WATCHDOG | user={user_id} | Sent waiting message after {WATCHDOG_TIMEOUT_SECONDS}s")
...
class EmpathicFallback:
    ...
    @staticmethod
    def get_fallback(lang: str, context_type: str, state: str = "", 
...
class DeadStateDetector:
    ...
    REQUIRED_HANDLERS = {
        "start", "name", "confirm_name", "birth_date", "nationality",
        "current_country", "current_city", "email", "phone",
        "education_level", "education_status", "education_field", "education_career",
        "work_status", "profession", "work_experience", "linkedin",
        "english_level", "visa_history", "visa_details", "visa_rejections",
        "criminal_record", "health_conditions", "savings",
        "family_status", "family_count", "family_member_relation",
        "family_member_name", "family_member_birth",
        "migration_reason", "timeline", "destination_preference",
        "climate_preference", "city_size_preference", "budget_initial",
        "select_country", "select_visa", "select_state", "select_city"
    }
...
def guaranteed_response(fallback_type: str = "handler_error"):
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            try:
                from app.services.telegram_bot import get_user_data, get_state
                if user_id:
            ...
            try:
                ...
                result = await asyncio.wait_for(
                    func(*args, **kwargs),
                    timeout=MAX_RESPONSE_TIME_SECONDS
                )
                response_sent = True
...
async def ensure_response(update, context, handler_func: Callable, 
                          fallback_type: str = "handler_error"):
    """
