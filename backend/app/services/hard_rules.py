...
class RuleCheckResult:
    ...
    rollback_to_phase: str | None = None
...
class HardRulesGuardian:
    ...
    def _check_no_form_or_phases_start(self, bot_response: str, interaction_count: int) -> RuleCheckResult:
        """
        REGLA 0: ❌ Prohibido iniciar con formularios o "FASES".
    ...
    def _check_no_personal_data_early(self, current_phase: str, bot_response: str) -> RuleCheckResult:
        """
    ...
    def _check_no_visa_without_profile(self, next_phase: str, context: dict[str, Any]) -> RuleCheckResult:
        """
        REGLA 2: ❌ Prohibido recomendar visa sin perfil completo.
        """
    ...
    def _check_no_options_without_summary(self, next_phase: str, context: dict[str, Any]) -> RuleCheckResult:
        """
        REGLA 3: ❌ Prohibido saltar a "opciones" sin resumen confirmado.
        """
    ...
    def _check_form_limit(
        self, interaction_count: int, form_count: int, last_form_interaction: int
    ) -> RuleCheckResult:
        """
        REGLA 4: ❌ Prohibido más de 1 formulario cada 5 interacciones.
        """
    ...
    def _check_no_advance_on_doubt(
        self, user_text: str, current_phase: str, next_phase: str
    ) -> RuleCheckResult:
        """
        REGLA 5: ❌ Prohibido avanzar si usuario expresa duda/corrección/confusión.
        """
    ...
    def check_state_transition(
        self, current_state: str, next_state: str, context: dict[str, Any]
    ) -> RuleCheckResult:
        """
    ...
    def _check_no_skipped_states(self, current_state: str, next_state: str) -> RuleCheckResult:
        """
    ...
    def _check_confirmation_required(self, next_state: str, context: dict[str, Any]) -> RuleCheckResult:
        """
    ...
    def _check_required_data_for_state(self, next_state: str, context: dict[str, Any]) -> RuleCheckResult:
        """
    ...
    def get_next_mandatory_state(self, current_state: str) -> str | None:
        """
    ...
    def check_form_prerequisites(
        self, interaction_count: int, has_dialogue: bool, has_empathy: bool, has_explanation: bool
    ) -> RuleCheckResult:
        """
    ...
    def detect_user_correction(self, user_text: str) -> tuple[bool, str | None]:
        """
        ...
        for pattern in self.CORRECTION_PATTERNS:
            if re.search(pattern, text_lower):
                ...
                if any(w in text_lower for w in ["familia", "solo", "esposa", "hijos", "pareja"]):
                    ...
                    w in text_lower
                    for w in ["trabajo", "profesión", "ingeniero", "contador", "abogado", "médico"]
                ):
    ...
    def detect_off_topic_response(self, user_text: str, expected_topic: str) -> tuple[bool, str | None]:
        """
    ...
    def handle_correction(
        self, user_text: str, correction_type: str, current_understanding: dict[str, Any], lang: str = "es"
    ) -> dict[str, Any]:
        """
    ...
    def handle_off_topic(
        self, user_text: str, detected_intention: str, current_phase: str, lang: str = "es"
    ) -> dict[str, Any]:
        """
        ...
        if lang == "es":
            if detected_intention == "user_question":
                ...
                return {
                    "response": ("Entiendo que quieres hablar de eso. 💬\n\n" "Cuéntame, te escucho."),
    ...
    def handle_audio_message(self, transcription: str, lang: str = "es") -> dict[str, Any]:
        """
    ...
    def check_response_has_explanation(self, bot_response: str, lang: str = "es") -> RuleCheckResult:
        """
        ...
        has_explanation = any(re.search(pattern, response_lower) for pattern in patterns)
        ...
        has_empathy = any(re.search(pattern, response_lower) for pattern in empathy_patterns)
    ...
    def check_no_visa_names_before_summary(
        self, bot_response: str, understanding_confirmed: bool
    ) -> RuleCheckResult:
        """
    ...
    def check_visa_context_sufficient(self, context: dict[str, Any]) -> RuleCheckResult:
        """
    ...
    def check_visa_presented_as_path(self, bot_response: str, lang: str = "es") -> RuleCheckResult:
        """
        ...
        mentions_visa = any(re.search(pattern, bot_response, re.IGNORECASE) for pattern in self.VISA_NAMES)

        if mentions_visa:
            has_path_language = any(re.search(pattern, response_lower) for pattern in path_patterns)
    ...
    def check_recommendation_has_required_elements(
        self, bot_response: str, lang: str = "es"
    ) -> RuleCheckResult:
        """
        ...
        is_recommendation = any(
            re.search(pattern, bot_response, re.IGNORECASE) for pattern in self.VISA_NAMES
        )
        ...
        for element, patterns in element_patterns.items():
            has_element = any(re.search(pattern, response_lower) for pattern in patterns)
            if not has_element:
    ...
    def validate_visa_recommendation(
        self, bot_response: str, context: dict[str, Any], lang: str = "es"
    ) -> RuleCheckResult:
        """
    ...
    def get_safe_visa_language(self, visa_type: str, lang: str = "es") -> dict[str, str]:
        """
    ...
    def validate_understanding_summary(
        self, summary_text: str, context: dict[str, Any], lang: str = "es"
    ) -> RuleCheckResult:
        """
        ...
        if missing:
            ...
            return RuleCheckResult(
                passed=False, violation=violation, message=f"Resumen incompleto. Falta: {', '.join(missing)}"
            )

        return RuleCheckResult(passed=True)

    def check_summary_confirmed(self, context: dict[str, Any]) -> RuleCheckResult:
        """
    ...
    def detect_bypass_attempt(self, action: str, context: dict[str, Any]) -> RuleCheckResult:
        """
    ...
    def _log_violation(self, violation: RuleViolation, context: str):
        ...
        logger.warning(f"🔒 REGLA VIOLADA: {violation.value} | " f"Contexto: {context} | " f"Bloqueante: Sí")
...
def can_advance_phase(
    current_phase: str, next_phase: str, user_text: str, context: dict[str, Any]
) -> tuple[bool, str]:
    """
