...
class ConversationRecorder:
    ...
    def start_session(self, user_id: int, language: str = "es") -> str:
        ...
        self._sessions[user_id] = ConversationSession(
            user_id=user_id, session_id=session_id, start_time=datetime.now().isoformat(), language=language
        )
    ...
    def record_user_message(
        ...
        friction_tags = FrictionDetector.detect_friction(
            text,
            prev_bot_msg=(
                self._recent_bot_msgs.get(user_id, [""])[-1] if self._recent_bot_msgs.get(user_id) else ""
            ),
        ...
        for tag in friction_tags:
            ...
            if tag in [FrictionTag.LOOP, FrictionTag.CONFUSION]:
                if self._consecutive_friction[user_id][tag_name] >= 2:
                    ...
                    logger.warning(
                        f"⚠️ CLARIFY_FORCED | user={user_id} | tag={tag_name} | count={self._consecutive_friction[user_id][tag_name]}"
                    )
        ...
        for tag in friction_tags:
            session.friction_summary[tag.value] = session.friction_summary.get(tag.value, 0) + 1
        ...
        logger.info(
            f"📝 USER_MSG | user={user_id} | state={state} | intent={intent} | friction={[t.value for t in friction_tags]} | block={should_block_data}"
        )
    ...
    def end_session(self, user_id: int, completed: bool = False) -> ConversationSession | None:
        ...
        logger.info(
            f"📹 SESSION_END | user={user_id} | duration={session.duration_seconds:.1f}s | completed={completed}"
        )
    ...
    def get_session(self, user_id: int) -> ConversationSession | None:
...
"""
        ...
        for msg in session.get("messages", []):
            ...
            md_content += (
                f"[{timestamp}] {sender}: {msg['text'][:100]}{'...' if len(msg['text']) > 100 else ''}\n"
            )
