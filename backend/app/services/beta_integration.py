...
from .beta_tracker import (
    get_beta_tracker, log_event, EventType, is_beta_mode,
    request_feedback, process_feedback, BETA_MODE
)
from .phase_manager import Phase, get_phase_manager
...
def load_beta_config() -> Dict[str, Any]:
    ...
    try:
        if BETA_CONFIG_PATH.exists():
            with open(BETA_CONFIG_PATH, 'r') as f:
                return json.load(f)
...
def get_beta_welcome_message() -> str:
    ...
    config = load_beta_config()
    return config.get("recruitment_message", """
...
def get_beta_completion_message() -> str:
    ...
    config = load_beta_config()
    return config.get("completion_bonus", """
...
def on_data_extracted(user_id: int, field: str, value: str, phase: str):
    ...
    tracker.log_event(
        ...
        phase,
        {"field": field, "value_length": len(str(value))}
    )
...
def handle_feedback_response(user_id: int, message: str) -> Optional[str]:
