...
class FormThrottler:
    ...
    def record_interaction(self, user_id: int, is_form: bool = False):
        ...
        self._user_interactions[user_id] = [
            t for t in self._user_interactions[user_id] if t > cutoff
        ]
        self._user_forms[user_id] = [
            t for t in self._user_forms[user_id] if t > cutoff
        ]
...
class InputInterpreter:
    ...
    PATTERNS = {
        ...
        InputType.CORRECTION: [
            r"corrijo", r"corrección", r"correccion", r"me equivoqué",
            r"me equivoque", r"quise decir", r"en realidad", r"no es",
            r"correction", r"i meant", r"actually", r"sorry.*wrong",
            r"perdón.*mal", r"perdon.*mal"
        ],
        InputType.CONCERN: [
        ...
        InputType.CONFIRMATION: [
            r"^sí$", r"^si$", r"^yes$", r"^ok$", r"^okay$", r"^vale$",
            r"^claro$", r"^correcto$", r"^exacto$", r"^así es$", r"^eso es$",
            r"^de acuerdo$", r"^perfecto$", r"^bien$", r"^sure$", r"^right$",
            r"^that's right$", r"^exactly$"
        ],
        InputType.REJECTION: [
            r"^no$", r"^nope$", r"^nah$", r"^después$", r"^despues$",
            r"^luego$", r"^ahora no$", r"^not now$", r"^later$", r"^maybe later$",
            r"^no gracias$", r"^no thanks$"
        ],
        InputType.GREETING: [
            r"^hola", r"^hello", r"^hi$", r"^hey", r"^buenos días",
            r"^buenas tardes", r"^buenas noches", r"^good morning",
            r"^good afternoon", r"^good evening"
        ],
        InputType.GRATITUDE: [
            r"gracias", r"thank", r"thanks", r"te agradezco", r"muy amable",
            r"appreciate"
        ],
        InputType.FRUSTRATION: [
            r"no entiendo", r"no comprendo", r"estoy confundido", r"confusa",
            r"ya te dije", r"otra vez", r"de nuevo", r"don't understand",
            r"confused", r"already told you", r"again"
        ],
        InputType.PERSONAL_INFO: [
            r"me llamo", r"mi nombre es", r"soy de", r"tengo \d+ años",
            r"trabajo como", r"trabajo en", r"mi profesión", r"mi profesion",
            r"my name is", r"i'm from", r"i am from", r"i'm \d+ years",
            r"i work as", r"i work at"
        ]
    }
    ...
    DATA_EXTRACTORS = {
        ...
        "age": [
            r"tengo (\d+) años", r"(\d+) años", r"i'm (\d+)", r"i am (\d+)",
            r"(\d+) years old"
        ],
        "profession": [
            r"trabajo como (\w+)", r"soy (\w+) de profesión",
            r"mi profesión es (\w+)", r"i work as (?:a |an )?(\w+)",
            r"i'm (?:a |an )?(\w+)"
        ],
        "country": [
            r"soy de (\w+)", r"vengo de (\w+)", r"i'm from (\w+)",
            r"i am from (\w+)", r"i come from (\w+)"
        ],
        "family_count": [
            r"(\d+) hijos", r"(\d+) niños", r"familia de (\d+)",
            r"(\d+) children", r"(\d+) kids", r"family of (\d+)"
        ]
    }
    ...
    def interpret(text: str, current_state: str = "", expected_type: str = "") -> InterpretedInput:
        ...
        result.should_advance = result.input_type in [
            InputType.DIRECT_ANSWER, InputType.CONFIRMATION, InputType.PERSONAL_INFO
        ]
        
        # Determinar si requiere clarificación
        result.requires_clarification = result.input_type in [
            InputType.QUESTION, InputType.CONCERN, InputType.FRUSTRATION
        ]
...
class UnderstandingGatekeeper:
    ...
    def get_status(self, user_id: int) -> UnderstandingStatus:
        ...
        if user_id not in self._user_understanding:
            self._user_understanding[user_id] = UnderstandingStatus(
                level=UnderstandingLevel.NOT_CONFIRMED
            )
    ...
    def get_next_required_question(user_data: Dict[str, Any], lang: str = "es") -> Tuple[str, str]:
        ...
        questions = {
            ...
            "en": {
                ...
                "criminal_record": ("Do you have any legal background I should know about?", "criminal_record")
            }
        }
        ...
        priority_order = ["name", "age", "nationality", "family_status", "family_details", 
                         "migration_reason", "visa_history", "criminal_record"]
...
class ConversationDirector:
    ...
    def record_exchange(self, user_id: int, user_input: str, bot_response: str, 
                        was_form: bool = False, state: str = ""):
    ...
    def get_next_action(self, user_id: int, user_data: Dict[str, Any], 
                        current_state: str, user_input: str) -> Dict[str, Any]:
        """
