...
class GlobalExceptionHandler:
    ...
    def log_error(self, user_id: int, state: str, error: Exception, context: str = ""):
        ...
        logger.error(f"🔴 ERROR | user={user_id} | state={state} | error={type(error).__name__}: {str(error)}")
...
class ResponseRotator:
    ...
    def select_template(self, user_id: int, templates: List[Tuple[str, str]], 
                        default_idx: int = 0) -> Tuple[str, str]:
        if not templates:
            return ("default", "")
        recent = self.get_recent_templates(user_id)
