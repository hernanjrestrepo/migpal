...
class NameValidatorV32:
    ...
    @staticmethod
    def extract_name_from_signal(text: str) -> str | None:
        """
...
class MandatoryConfirmation:
    ...
    def get_pending_confirmation(self, user_id: int) -> dict[str, Any] | None:
        """Obtener confirmación pendiente para un usuario"""
        return self._pending_confirmations.get(user_id)

    def confirm_field(self, user_id: int) -> tuple[str, Any] | None:
        """
        ...
        if pending:
            logger.info(
                f"✅ FIELD_CONFIRMED | user={user_id} | field={pending['field']} | value={pending['value']}"
            )
            return pending["field"], pending["value"]
        return None

    def reject_field(self, user_id: int) -> str | None:
        """
...
class AntiLoopRotator:
    ...
    def __new__(cls):
        if cls._instance is None:
            cls._instance = super().__new__(cls)
            cls._instance._user_state = (
                {}
            )  # user_id -> {last_intent, last_hash, repeat_count, used_templates}
        return cls._instance
    ...
    def record_bot_response(self, user_id: int, intent: str, message: str):
        ...
        if state["last_intent"] == intent and state["last_hash"] == msg_hash:
            ...
            logger.warning(
                f"⚠️ LOOP_DETECTED | user={user_id} | intent={intent} | repeat_count={state['repeat_count']}"
            )
        else:
            state["repeat_count"] = 1
