...
class BetaTracker:
    ...
    def _update_metrics(self, event: BetaEvent):
        ...
        if event.event_type == EventType.SESSION_START.value:
            ...
            if question in self._question_history[event.user_id]:
                ...
                self.log_event(event.user_id, EventType.QUESTION_REPEATED, event.phase, {"question": question})
            else:
    ...
    def get_user_metrics(self, user_id: int) -> Optional[UserMetrics]:
    ...
    def get_active_users(self) -> List[int]:
        ...
        return [
            uid for uid, m in self._user_metrics.items()
            if not m.abandoned_at_phase
        ]
    ...
    def get_completed_users(self) -> List[int]:
        ...
        return [
            uid for uid, m in self._user_metrics.items()
            if "CIERRE" in m.phases_completed
        ]
    ...
    def get_friction_points(self) -> List[FrictionPoint]:
        ...
        phase_help_requests: Dict[str, List[int]] = {}
    ...
    def get_repeated_questions(self) -> Dict[str, int]:
        ...
        for uid, questions in self._question_history.items():
            for q in questions:
...
_beta_tracker: Optional[BetaTracker] = None
...
def get_user_metrics(user_id: int) -> Optional[UserMetrics]:
...
def process_feedback(user_id: int, message: str) -> Tuple[bool, str]:
    ...
    try:
        ...
        if BETA_FEEDBACK_FILE.exists():
            with open(BETA_FEEDBACK_FILE, 'r') as f:
