...
def get_recommended_routes(
    ...
    if not profile:
        ...
        processes = session.exec(select(MigrationProcess).where(MigrationProcess.is_active is True)).all()
        return processes

    # Filter processes based on user's profile
    query = select(MigrationProcess).where(
        MigrationProcess.is_active is True, MigrationProcess.country_to == profile.target_country
    )
...
def list_migration_processes(
    country_to: str | None = Query(None),
    visa_type: str | None = Query(None),
    max_cost: float | None = Query(None),
    max_time_months: int | None = Query(None),
    session: Session = Depends(get_session),
):
    """
    List available migration processes with optional filters
    """
    query = select(MigrationProcess).where(MigrationProcess.is_active is True)
...
def get_migration_process(process_id: int, session: Session = Depends(get_session)):
    """
...
def select_migration_process(
    ...
    return {"message": "Migration process selected successfully", "process": process, "profile": profile}
...
def get_my_migration_process(
    ...
    if not profile.selected_process_id:
        return {"message": "No migration process selected yet", "profile": profile, "process": None}

    process = session.get(MigrationProcess, profile.selected_process_id)

    return {"profile": profile, "process": process, "current_stage": profile.current_stage}
...
def update_migration_stage(
    ...
    if new_stage not in valid_stages:
        raise HTTPException(
            status_code=400, detail=f"Invalid stage. Must be one of: {', '.join(valid_stages)}"
        )
    ...
    return {"message": "Stage updated successfully", "current_stage": profile.current_stage}
...
def get_migration_timeline(
    ...
    if not profile.selected_process_id:
        raise HTTPException(status_code=404, detail="No migration process selected. Select a route first.")
