...
HUMAN_CONVERSATIONS = [
    ...
    # 2. Correcciones
    {"type": "correction", "inputs": ["me llamo Juan", "no, perdón, me llamo Pedro", "mi nombre es Pedro García"]},
    
    # 3. Off-topic
    {"type": "off_topic", "inputs": ["¿qué hora es?", "cuéntame un chiste", "¿cuál es tu película favorita?"]},
    
    # 4. Respuestas muy cortas
    {"type": "short", "inputs": ["si", "no", "ok", "ya", "mm"]},
    
    # 5. Respuestas muy largas
    {"type": "long", "inputs": ["Bueno mira, te cuento que yo soy ingeniero de sistemas con más de 15 años de experiencia trabajando en empresas multinacionales, tengo una maestría en administración de empresas y actualmente gano alrededor de 8000 dólares mensuales, estoy casado y tengo 2 hijos de 5 y 8 años, mi esposa es médica y queremos migrar porque la situación económica en nuestro país está muy difícil"]},
    
    # 6. Preguntas antes de responder
    {"type": "question_first", "inputs": ["¿para qué necesitas mi nombre?", "¿es seguro dar mis datos?", "¿quién va a ver esta información?"]},
    
    # 7. Frustración
    {"type": "frustration", "inputs": ["ya te dije mi nombre", "no entiendo qué quieres", "esto es muy confuso", "otra vez lo mismo?"]},
    
    # 8. Cambio de idioma
    {"type": "language_switch", "inputs": ["I want to migrate", "quiero ir a USA", "how much does it cost?"]},
    
    # 9. Emojis y símbolos
    {"type": "emoji", "inputs": ["👋 hola!", "🇨🇴 soy de Colombia", "💰 tengo $50,000", "✈️ quiero viajar pronto"]},
    ...
    {"type": "dates", "inputs": ["nací el 15/03/1990", "nací en marzo de 1990", "tengo 34 años", "1990-03-15"]},
    
    # 12. Información parcial
    {"type": "partial", "inputs": ["Juan", "Colombia", "ingeniero"]},
    
    # 13. Información con contexto
    {"type": "context", "inputs": ["mi nombre completo es Juan Carlos Pérez García", "vivo en Bogotá, Colombia desde hace 10 años"]},
    
    # 14. Negaciones
    {"type": "negation", "inputs": ["no tengo hijos", "no estoy casado", "no tengo visa", "nunca he viajado a USA"]},
    ...
    {"type": "process_questions", "inputs": ["¿cuánto cuesta?", "¿cuánto tiempo toma?", "¿qué documentos necesito?"]},
    ...
    {"type": "currencies", "inputs": ["tengo 200 millones de pesos", "gano 5000 euros", "ahorro $3000 al mes"]},
    
    # 24. Profesiones complejas
    {"type": "professions", "inputs": ["soy ingeniero de software senior", "trabajo como gerente de proyectos IT", "soy médico especialista en cardiología"]},
    
    # 25. Situaciones familiares complejas
    {"type": "family", "inputs": ["viajo con mi esposa y 3 hijos", "somos 5: yo, mi esposa, 2 hijos y mi mamá", "viajo solo pero mi familia viene después"]},
    
    # 26. Urgencia
    {"type": "urgency", "inputs": ["necesito irme ya", "es urgente", "tengo que salir este mes", "no puedo esperar"]},
    
    # 27. Dudas sobre visas
    {"type": "visa_questions", "inputs": ["¿qué visa me conviene?", "¿puedo trabajar con visa de turista?", "¿cuánto dura la visa H1B?"]},
    
    # 28. Comparaciones
    {"type": "comparisons", "inputs": ["¿es mejor Miami o Houston?", "¿qué diferencia hay entre H1B y L1?", "¿conviene más estudiar o trabajar?"]},
    
    # 29. Silencio (timeout)
    {"type": "silence", "inputs": []},
    
    # 30. Secuencia completa realista
    {"type": "realistic", "inputs": [
        "hola",
...
class ConversationSimulator:
    ...
    async def simulate_input(self, text: str, user_id: int = 12345) -> Dict[str, Any]:
        ...
        try:
            ...
            from app.services.flow_governor import interpret_user_input, InputType
            from app.services.memory_profiler import store_user_input, detect_correction
...
async def main():
    ...
    # Tasa de respuesta
    response_rate = (analysis['responded'] / analysis['total_inputs'] * 100) if analysis['total_inputs'] > 0 else 0
