...
from app.services.profile_checklist import (
    get_profile_checklist, check_profile_completeness, ChecklistCategory
)
from app.services.test_time_reasoning import (
    get_test_time_reasoner, reason_visa_analysis, reason_migration_plan,
    check_inconsistencies, ReasoningTask, DraftApproach
)
from app.services.coherence_validator import (
    get_coherence_validator, validate_response, is_response_safe
)
...
async def test_profile_checklist():
    ...
    complete_user = {
        ...
        "language": "es",
        "profile": {
            "personal": {"name": "María", "age": 35, "has_family": True},
            "education": {"level": "Maestría"},
            "work": {"profession": "Doctora", "experience_years": 10},
            "languages": {"english": "Avanzado"},
            "financial": {"savings": "$50,000"},
            "history": {"visa_history": "B1/B2", "visa_denials": False, "legal_issues": False},
            "migration": {"reason": "Trabajo", "timeline": "1 año"}
        }
    }
...
async def test_test_time_reasoning():
    ...
    print_result(
        "Múltiples drafts generados",
        len(result.all_drafts) >= 2,
        f"Total drafts: {len(result.all_drafts)}"
    )
    ...
    print_result(
        ...
        len(invented) == 0,
        f"Inventados: {invented if invented else 'Ninguno'}"
    )
    ...
    print("\n--- Draft Seleccionado ---")
    print(result.selected_draft.content[:500] + "..." if len(result.selected_draft.content) > 500 else result.selected_draft.content)
...
async def test_coherence_validator():
    ...
    print_result(
        ...
        len([i for i in result.issues if i.category == "contradiction"]) > 0 or 
        len([i for i in result.issues if i.category == "invented_data"]) > 0,
