...
class ConversationAnalyzer:
    ...
    def detect_loop_bug(self, session: Dict) -> Optional[Bug]:
            ...
            if text1 == text2 == text3:
                ...
                return Bug(
                    ...
                    reproduction_steps=[
                        ...
                        f"3. Observe bot response repeating",
                        f"4. Send another message: '{user_msgs[1]['text'] if len(user_msgs) > 1 else 'N/A'}'",
                        f"5. Bot repeats same response again"
                    ],
                    context={
                        "repeated_message": msg1['text'],
                        "user_inputs": [m['text'] for m in user_msgs]
                    }
                )
        
        return None
    
    def detect_phantom_data_bug(self, session: Dict) -> Optional[Bug]:
    ...
    def detect_confusion_bug(self, session: Dict) -> Optional[Bug]:
                ...
                if confusion_count >= 3:
                    return Bug(
                        ...
                        reproduction_steps=[
                            ...
                            f"2. User messages indicating confusion:",
    ...
    def detect_rage_quit(self, session: Dict) -> Optional[Bug]:
            ...
            if 'rage' in friction_tags:
                ...
                if remaining <= 2:
                    return Bug(
                        ...
                        bug_type="RAGE_QUIT",
                        severity="high",
                        description=f"User abandoned after expressing frustration",
                        user_id=session['user_id'],
                        session_id=session['session_id'],
                        timestamp=msg['timestamp'],
                        state=msg.get('state', 'unknown'),
                        reproduction_steps=[
                            ...
                            f"3. User abandoned conversation shortly after"
                        ],
                        context={
    ...
    def detect_stuck_state(self, session: Dict) -> Optional[Bug]:
                    ...
                    return Bug(
                        ...
                        reproduction_steps=[
                            ...
                            *[f"   - '{m['text']}'" for m in user_msgs[:3]],
                            f"3. User remains stuck in same state"
                        ],
                        context={
                            "state": state,
...
"""
    ...
    if report.bugs_found:
        for bug in sorted(report.bugs_found, key=lambda b: {'critical': 0, 'high': 1, 'medium': 2, 'low': 3}[b.severity]):
