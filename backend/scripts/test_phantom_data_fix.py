...
class PhantomDataTester:
    ...
    def run_all_tests(self) -> dict[str, Any]:
        ...
        for test_func in tests:
            try:
                ...
                self.results.append(
                    TestResult(test_name=test_func.__name__, passed=False, details=f"Exception: {str(e)}")
                )
    ...
    async def run_simulation(self, profile: dict[str, Any]) -> SimulationResult:
        ...
        user_id = profile["id"]
        result = SimulationResult(user_id=user_id, profile_name=profile.get("name", "Unknown"), success=False)
...
"""
    ...
    for result in test_results["results"]:
        ...
        if result["phantom_data_detected"]:
            report += "- **⚠️ Phantom Data Detected**\n"
        if result["loop_detected"]:
            report += "- **⚠️ Loop Detected**\n"
        report += "\n"
...
async def main():
    ...
    print(
        f"Simulations: {simulation_results['successful']}/{simulation_results['total_simulations']} successful"
    )
