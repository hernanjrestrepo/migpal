...
async def test_segment_3_responses():
    ...
    response = script.process_segment_3(
        "Mi esposa y mis dos hijos de 5 y 8 años",
        {},
        "es"
    )
    
    checks = [
        ("hijos" in response.message.lower() or "pequeños" in response.message.lower(), 
         "Menciona hijos"),
        ("opciones" in response.message.lower() or "visa" in response.message.lower() or "escuelas" in response.message.lower(),
