...
CLIENT_MESSAGES = [
    ...
    {"type": "message", "text": "Mi presupuesto es de unos 15000 dólares", "description": "Información financiera"},
]
...
def analyze_response(response: str, turn: int) -> dict:
    ...
    if response:
        empathy_words = ["entiendo", "comprendo", "tranquilo", "normal", "ayudo", "aquí", "juntos", "paso a paso"]
...
async def run_simulation():
    ...
    for msg_config in CLIENT_MESSAGES:
        ...
        try:
            ...
            try:
                ...
                log(f"Timeout waiting for response (30s)", "WARN")
            ...
            if captured_responses:
                ...
                add_transcript(turn, "BOT", response, new_state, {
                    "has_buttons": len(captured_buttons) > 0,
                ...
                if is_emoji_only_response(response):
                    ...
                    RESULTS["diagnostics"]["frictions"].append(f"Turn {turn}: Emoji-only response (no text content)")
                    log(f"Emoji-only response detected!", "FAIL")
                    all_passed = False
                
                # V4.2.1 FIX D: Check for state regression
                if check_state_regression(previous_state, new_state, turn):
                    RESULTS["diagnostics"]["state_regressions"].append(f"Turn {turn}: {previous_state} → {new_state}")
                    RESULTS["diagnostics"]["frictions"].append(f"Turn {turn}: State regression ({previous_state} → {new_state})")
                ...
                if forbidden:
                    ...
                    RESULTS["diagnostics"]["frictions"].append(f"Turn {turn}: Forbidden pattern(s) found: {forbidden}")
                ...
                if len(captured_responses) > 3:
                    ...
                    RESULTS["diagnostics"]["frictions"].append(f"Turn {turn}: Multiple responses ({len(captured_responses)}) - potential loop")
                    log(f"Multiple responses detected: {len(captured_responses)}", "WARN")
                elif len(captured_responses) == 3:
                    log(f"3 responses (normal for onboarding flow)", "INFO")
    ...
    if total_turns > 0:
        RESULTS["diagnostics"]["engagement_score"] = min(10, (RESULTS["diagnostics"]["empathy_score"] / total_turns) * 5 + 5)
    ...
    if not all_passed or RESULTS["diagnostics"]["forbidden_patterns_found"] or RESULTS["diagnostics"]["errors"] or has_emoji_only or has_state_regression:
        RESULTS["verdict"] = "NO-GO"
...
def print_results():
    ...
    for entry in RESULTS["transcript"]:
        ...
        print(f"{role_icon} {entry['role']}: {entry['message'][:300]}{'...' if len(entry['message']) > 300 else ''}")
    ...
    validations = [
        ...
        ("Exceptions", "Exception" not in str(diag["forbidden_patterns_found"]) and "AttributeError" not in str(diag["forbidden_patterns_found"])),
