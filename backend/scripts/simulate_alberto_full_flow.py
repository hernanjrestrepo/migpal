...
def get_perfect_response(phase: str, turn: int) -> str:
    ...
    phase_map = {
        "registro": 1, "diagnostico": 2, "perfilamiento": 3, "visa": 4,
        "estado": 5, "ciudad": 6, "barrio": 7, "vivienda": 8,
        "colegio": 9, "timeline": 10, "presupuesto": 11, "cierre": 12
    }
...
def run_simulation():
    ...
    phases = [
        "registro",  # 1
        "diagnostico", "diagnostico", "diagnostico", "diagnostico", "diagnostico", "diagnostico",  # 2-7
        "perfilamiento", "perfilamiento", "perfilamiento", "perfilamiento",  # 8-11
        "visa", "visa", "visa",  # 12-14
        "estado", "estado",  # 15-16
        "ciudad", "ciudad",  # 17-18
        "barrio", "barrio",  # 19-20
        "vivienda", "vivienda",  # 21-22
        "colegio", "colegio",  # 23-24
        "timeline",  # 25
        "presupuesto",  # 26
        "cierre",  # 27
    ]
    ...
    for turn, phase in enumerate(phases, 1):
        ...
        print(f"[{turn:02d}] {phase:12s} {status} | L:{evaluation['line_count']} Q:{evaluation['question_count']} P:{'✓' if evaluation['has_progress'] else '✗'} M:{'✓' if evaluation['has_micro_check'] else '✗'} | {issues_str}")
    ...
    print(f"  • micro_check_rate: {micro_rate:.1f}% {'✅' if micro_rate >= 70 else '⚠️'} (turnos con pregunta)")
