...
def test_llm_freedom():
    ...
    interactions = [
        (
            "Hola, quiero migrar",
            "¡Hola! Qué gusto saludarte. Cuéntame, ¿qué te motiva a pensar en migrar?",
            {},
        ),
        (
            "Es que mi país está muy difícil",
            "Entiendo perfectamente. La situación puede ser muy frustrante. ¿Qué es lo que más te preocupa?",
            {"emotional_state": "frustrated"},
        ),
        (
            "Todo, el trabajo, la seguridad",
            "Es comprensible. Son preocupaciones muy válidas. ¿Tienes familia que migre contigo?",
            {"concerns": ["work", "security"]},
        ),
    ]
...
def test_no_forced_forms():
    ...
    for i in range(4):
        decision = process_llm_response(layer, f"Mensaje {i}", f"Respuesta {i}", {})
...
def test_visa_blocked_without_summary():
    ...
    process_llm_response(layer, "Sí, es correcto", "Perfecto, gracias por confirmar.", {})
...
def test_correction_stops_progress():
    ...
    # Usuario corrige
    decision = process_llm_response(
        layer, "No, en realidad viajo con mi esposa", "Entendido, gracias por la aclaración.", {}
    )
...
def test_confirmation_advances():
    ...
    # Usuario confirma
    decision = process_llm_response(layer, "Sí, es correcto", "Perfecto, gracias por confirmar.", {})
