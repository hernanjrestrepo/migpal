...
class ManualSimulator:
    ...
    async def _run_standard_scenario(self, user: dict):
        ...
        await self._simulate_turn(
            user_input=None, callback="lang_es", expected_phase="greeting", description="Seleccionar idioma"
        )
        ...
        # 4. NOMBRE
        await self._simulate_turn(
            user_input="Carlos Rodríguez", expected_phase="greeting", description="Dar nombre"
        )
        ...
        await self._simulate_turn(
            user_input=None, callback="flow_work", expected_phase="work", description="Elegir trabajo"
        )
    ...
    async def _run_correction_scenario(self, user: dict):
        ...
        # 2. RESPUESTA
        await self._simulate_turn(
            user_input="Quiero trabajar en USA", expected_phase="motivation", description="Motivación inicial"
        )

        # 3. CONSENTIMIENTO
        await self._simulate_turn(
            user_input=None, callback="onboarding_yes", expected_phase="greeting", description="Aceptar"
        )

        # 4. NOMBRE INCORRECTO
        await self._simulate_turn(
            user_input="Juann", expected_phase="greeting", description="Nombre con typo"
        )
    ...
    async def _simulate_turn(
        ...
        from app.services.case_storage import load_user_data
...
async def run_all_simulations():
    ...
    critical_violations = sum(
        len([v for v in r.violations if v.severity == "critical"]) for r in results.values()
    )
