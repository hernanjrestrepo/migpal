...
def test_segment_3_correction_detection():
    ...
    for text, should_detect, expected_type in test_cases:
        ...
        print(
            f"   {status} '{text[:35]}...' → {'CORRECCIÓN' if is_correction else 'normal'} ({correction_type})"
        )
...
def test_segment_3_audio_handling():
    ...
    transcription = (
        "Quiero migrar porque mi país está muy difícil y busco mejores oportunidades para mi familia"
    )
...
def test_segment_5_logging():
    ...
    if guardian.violations_log:
        ...
        checks = [
            ...
            (log.get("is_blocking") is True, "is_blocking = True"),
        ]
    else:
...
def main():
    ...
    results.append(
        ("S5: Resumen: paráfrasis+familia+motivación+restricciones", test_segment_5_summary_validation())
    )
