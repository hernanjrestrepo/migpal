...
class RulesValidator:
    ...
    async def validate_conversation(self, conversation_name: str, messages: List[str]) -> ValidationResult:
        ...
        for i, user_message in enumerate(messages):
            ...
            if "es correcto" in user_message.lower() and phase_before == ExplorationPhase.UNDERSTANDING_SUMMARY:
                self.understanding_confirmed = True
        
        # Verificar que se completó el flujo
        ctx_final = advisor.get_context(user_id)
    ...
    def _validate_rule_3(self, extracted, response, turn):
        ...
        if extracted:
            ...
            paraphrase_indicators = [
                "entiendo", "entonces", "perfecto", "excelente",
                "eres", "tienes", "cuentas con", "viajas"
            ]
    ...
    def _validate_rule_4(self, ctx, response, turn):
        ...
        recommendation_keywords = [
            "h-1b", "l-1", "o-1", "eb-1", "eb-2", "eb-3",
            "visa de trabajo", "te recomiendo", "la mejor opción",
            "deberías ir a", "california", "texas", "florida",
    ...
    def _validate_rule_5(self, user_message, phase_before, phase_after, turn):
        ...
        is_doubt = any(
            __import__('re').search(pattern, user_message.lower())
