...
def extract_name_from_input(user_input: str, validator) -> Tuple[bool, Optional[str], str]:
    """
...
def run_simulation(persona: Dict[str, Any]) -> SimulationResult:
    ...
    confirmation = get_mandatory_confirmation()
    anti_loop = get_anti_loop_rotator()
        ...
        if field_type == "name":
            ...
            if is_valid and name:
                ...
                if any(word in user_input.lower() for word in [
                    'ingeniero', 'médica', 'inglés', 'avanzado', 'intermedio',
                    'básico', 'ok', 'gracias', 'abogado', 'guitarrista', 'soy de'
                ]):
...
def run_all_simulations() -> Dict[str, Any]:
    ...
    if all_issues:
        ...
        report["top_5_causes"] = [
            ...
            for cause, count in issue_counts.most_common(5)
        ]
