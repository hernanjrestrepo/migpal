    ...
    async def send_message(self, text: str) -> dict:
        ...
        async with httpx.AsyncClient() as client:
            response = await client.post(
                f"{API_BASE}/sendMessage", json={"chat_id": self.chat_id, "text": text}
            )
    ...
    async def get_updates(self, timeout: int = 30) -> list:
        ...
        async with httpx.AsyncClient(timeout=timeout + 5) as client:
            response = await client.post(
                ...
                json={"offset": self.last_update_id + 1, "timeout": timeout, "allowed_updates": ["message"]},
            )
            data = response.json()
...
async def verify_bot_responses():
    ...
    print("📊 Análisis de logs (últimas 200 líneas):")
