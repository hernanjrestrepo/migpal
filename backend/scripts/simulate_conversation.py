...
def simulate_conversation():
    ...
    if questions_asked < turn_count * 0.3:
        print("   ⚠️ Pocas preguntas de seguimiento")
        ux_score -= 10

    if reflections_made < turn_count * 0.2:
        print("   ⚠️ Pocas reflexiones empáticas")
        ux_score -= 10

    if total_bot_chars / (turn_count + 1) > 400:
        print("   ⚠️ Respuestas promedio muy largas")
        ux_score -= 5
    ...
    if reflections_made >= turn_count * 0.5:
        print("   ✅ Buena empatía y reflexión")
        ux_score += 5

    if completion >= 50:
        print("   ✅ Buena extracción de información")
        ux_score += 10
