...
**Arquitectura vigente:** plataforma web con backend en arquitectura DDD/hexagonal (`backend/core/`: bounded contexts `identity`, `case_engine`, `conversation`, `decision_engine`, `policy_engine`, `recommendation`, `execution_plan`, cada uno con capas `domain/application/infrastructure/adapters`), Postgres + Redis vía Docker Compose (`docker compose up`, un solo comando), frontend estático (`frontend/public/*.html`, Next.js pospuesto hasta que haya más superficie). El bot de Telegram original (`backend/app/services/`) sigue existiendo como motor de IA conversacional; cada bounded context que necesita LLM tiene su propio adapter delgado (`ai_adapter.py`), nunca compartido entre bounded contextos (A-ADR-006).

**Estado de ejecución:** Recovery ✅ · Foundation ✅ · Sprint 1 – Hito 1 (Registro/Login/Case) ✅ · Sprint 1 – Hito 2 (Conversación + Assessment) ✅ · Sprint 1 – Hito 3 (Recommendation) ✅ **CERRADO — Recommendation Baseline v1.0 Frozen** (2026-07-31), con auditoría independiente aprobada ([docs/HITO_3_AUDIT.md](docs/HITO_3_AUDIT.md)) y revisión arquitectónica final ([docs/HITO_3_FINAL_CLOSE.md](docs/HITO_3_FINAL_CLOSE.md)) · Hito 4 (Execution Plan) ✅ **CERRADO** (2026-08-17), con auditoría independiente aprobada con observaciones ([docs/HITO_4_AUDIT.md](docs/HITO_4_AUDIT.md)) y cierre final ([docs/HITO_4_FINAL_CLOSE.md](docs/HITO_4_FINAL_CLOSE.md)).

**Último hito completado:** Hito 4 — una Recommendation ACCEPTED se convierte en un `ExecutionPlan` real: pasos derivados determinísticamente de sus `required_documents` y su `next_step`, con dependencias entre pasos (el paso final queda bloqueado hasta completar los de documentos), progreso persistente y un mensaje de cierre al completar todos los pasos. API completa (`POST`/`GET`/`complete`), frontend extendido en `caso.html` (sección 5, "Mi Plan"). Recorrido completo Discovery → Assessment → Recommendation → Execution verificado en navegador real contra Postgres y Ollama reales. Cerrado tras una auditoría independiente (APROBADO CON OBSERVACIONES) y una fase de cierre que corrigió el hallazgo mayor (descripciones de paso genéricas) y el hallazgo menor de invariante 4 sin validar; ver [docs/HITO_4_FINAL_CLOSE.md](docs/HITO_4_FINAL_CLOSE.md) para el detalle y los riesgos aceptados como deuda técnica.

**Próximo hito:** sin definir todavía — no se documenta contenido de Hito 5 hasta que haya una sesión de diseño para él (mismo criterio aplicado a los hitos anteriores).
